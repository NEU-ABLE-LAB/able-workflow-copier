...
1. [ ] `workflow/rules/docs_dag_svg.smk`
   1. [ ] Specify `input:` directives as needed, including the dry-run manifest passed to the script.
...
2. [ ] `workflow/scripts/rules_conda_DOCS/dag_svg.py`
   ...
   3. [ ] Load dry-run manifests from `data/tests/dry-run/` using `include:` and `touch:` entries.
...
3. [ ] `test/workflow/scripts/rules_conda_DOCS/test_dag_svg.py.jinja`
   ...
   2. [ ] Provide dry-run manifests under `data/tests/dry-run/` using `include:` and `touch:` as needed.
...
4. [ ] `tests/workflow/rules/test_snakemake_dag_svg.py`
   1. [ ] Modify `create_dummy_input_file()` to create the dummy subsets of data described by the dry-run manifests.
